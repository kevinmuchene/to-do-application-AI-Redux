...
import { Button, List, ListItem, Typography, Avatar } from "@mui/material";
...
import { DialogComponentProps, ToDo } from "../../common/interfaces/Interfaces";
        ...
        <DialogContent>
          {modalToDoList.length === 0 ? (
            <CustomAlert
              ...
              message={
                doneOrDeleted === "done"
                  ? "You Don't Have Any Completed Todo's"
                  : "You Don't Have Any Deleted Todo's"
              }
            />
          ) : (
            modalToDoList.map((todo) => <TodoList key={todo.id} todo={todo} />)
          )}
        </DialogContent>
        <DialogActions>
  ...
  return (
    <List sx={{ width: "100%", bgcolor: "background.paper" }}>
      <ListItem>
        ...
        <ListItemText
          primary={<Typography color={"secondary"}>{todo.title}</Typography>}
          secondary={
            <React.Fragment>
              <Typography
                sx={{ display: "inline" }}
